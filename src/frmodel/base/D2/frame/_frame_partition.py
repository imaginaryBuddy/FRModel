...
class _Frame2DPartition(ABC):
    ...
    def view_windows(self: 'Frame2D',
                     ...
                     height_step: int,
                     width_step: int,
                     as_frames: bool = True) -> Union[List[List['Frame2D']], np.ndarray]:
        """ Partitions the image into windows using striding

        Note that the parameters must cleanly divide the image.
        
        E.g. 200 x 200 cannot be 100 Step 3 x 100 Step 7

        If an np.ndarray is expected, the dims are
            [RowWindow, ColWindow, Row, Col, Channels]

        :param height: Height of expected window
        :param width: Width of expected window
        :param height_step: Step of height window
        :param width: Steo of width window
        :param as_frames: Whether to output as a list of Frame2D or a 5D np.ndarray
        """

        view: np.ndarray = view_as_windows(self.data, (height, width, self.shape[-1]),
                                           (height_step, width_step, 1)).squeeze(2)
        # [HW, WW, H, W, C]
        if as_frames:
            return [[Frame2D(view[h, w, 0], self.labels)
    ...
    def view_blocks(self: 'Frame2D',
        ...
        If an np.ndarray is expected, the dims are
            [RowBlock, ColBlock, Row, Col, Channels]
        ...
        :param as_frames: Whether to output as a list of Frame2D or a 5D np.ndarray
        """
        view: np.ndarray = view_as_blocks(self.data, (height, width, self.shape[-1])).squeeze(2)
        # [HW, WW, H, W, C]
        if as_frames:
            return [[Frame2D(view[h, w, 0], self.labels)
                     for h in range(view.shape[0])] for w in range(view.shape[1])]
